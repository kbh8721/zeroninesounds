  ...
  return (
    <section className="py-12 md:py-0 px-6 md:px-0 flex flex-col gap-8 h-full">
      ...
      <div className="hidden md:block widget">
        <div className="widget-title">진행 중인 세션</div>
        <div className="relative pl-6 mb-5 border-l-2 border-primary">
          <div className="text-xs font-semibold text-white">최종 믹스 리뷰</div>
        ...
        <div className="relative pl-6 mb-5 border-l-2 border-deep-gray">
          <div className="text-xs font-semibold text-text-secondary">아날로그 서밍</div>
        ...
        <div className="relative pl-6 border-l-2 border-deep-gray">
          <div className="text-xs font-semibold text-text-secondary">프로젝트 접수</div>
      ...
      <div className="md:widget md:flex-1 md:flex md:flex-col md:justify-center md:items-center md:border-dashed md:opacity-80 md:hover:opacity-100 transition-opacity">
        {step === 1 ? (
          ...
          >
            <h3 className="text-2xl font-semibold mb-8 md:hidden">1. 프로젝트 파일 업로드</h3>
            ...
            >
              {uploadComplete ? (
                <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="text-green-500 flex flex-col items-center">
                  ...
                  <span className="font-medium text-sm md:text-xs">업로드 완료</span>
                </motion.div>
              ) : isUploading ? (
                <div className="flex flex-col items-center text-primary">
                  ...
                  <span className="font-medium text-sm md:text-xs animate-pulse">파일 처리 중...</span>
                </div>
              ) : (
                <div className="flex flex-col items-center text-text-secondary hover:text-white transition-colors">
                  ...
                  <span className="font-medium text-white mb-2 md:hidden">여기를 클릭하거나 파일을 드래그하세요</span>
                  <span className="text-sm md:text-[10px] md:font-mono md:tracking-[1px]">파일 업로드</span>
                </div>
              )}
            </div>
            ...
            <div className="w-full max-w-md mt-12 md:hidden">
              <h4 className="text-sm font-mono text-gray-500 mb-4 uppercase tracking-wider">프로젝트 상태 미리보기</h4>
              <div className="space-y-4">
                {['파일 접수', '검토 중', '작업 중', '피드백 대기', '최종 전달'].map((status, i) => (
          ...
          >
            <h3 className="text-2xl font-semibold mb-8 text-center md:text-sm md:mb-4">2. 컨설팅 예약</h3>
            
            <div className="grid grid-cols-1 gap-8 md:gap-4">
              {/* Time Slots */}
              <div className="space-y-4 md:space-y-2">
                <h4 className="font-medium flex items-center gap-2 mb-6 md:mb-2 md:text-xs"><Clock className="w-4 h-4 text-primary" /> 예약 가능 시간</h4>
                ...
                <button 
                  onClick={() => alert('예약이 확정되었습니다! (프로토타입)')}
